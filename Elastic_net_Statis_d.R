Elastic_net_Statis <- function(X, Transform.Data = NULL, Alpha = 1e-04, Lambda = 1e-04) {
  ...
  for (i in 1:length(X)) {
    if (i != length(X)) {
      res <- identical(names(as.data.frame(X[[i]])), names(as.data.frame(X[[i+1]])))
      if (res != TRUE){
        stop("Col names are not the same") }
    }
  }
  n_col <- dim(X[[1]])[2]
  ...
  S <- list()
  for (i in 1:length(X)) {
    S[[i]] <- as.matrix(t(X[[i]])) %*% as.matrix(X[[i]])
  }
  names(S) <- names(X)
  ...
  # Compromise Matrix
  # -------------------------------------------------------
  SC <- matrix(0, nrow = n_col, ncol = n_col)
  for (i in 1:length(X)) {
    SC <- SC + (alpha[i] * S[[i]])
  }
  ...
  SC_spca_V <-
    spca(
      ...
      k = ncol(SC),
      alpha = Alpha,
      beta = Lambda)$loadings
  SC_svd <- svd(SC)
  F_ <- SC_spca_V %*% diag(SC_svd$d^0.5)
  rownames(F_) <- colnames(X[[1]])
  
  # -------------------------------------------------------
  # Coordinates
  # -------------------------------------------------------
  G <- SC_spca_V%*%diag(SC_svd$d^(-0.5))
  Coordinates <- list()
  for (i in 1:length(X)) {
